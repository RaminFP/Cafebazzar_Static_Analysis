        ...
        if (state.willRunPredictiveAnimations() && getChildCount() != 0 && !state.isPreLayout() && supportsPredictiveItemAnimations()) {
            ...
            while (i5 < size) {
                ...
                if (viewHolder.isRemoved()) {
                    ...
                    if (((viewHolder.getLayoutPosition() < position ? true : DEBUG) != this.mShouldReverseLayout ? -1 : 1) == -1) {
    ...
    private boolean updateAnchorFromPendingData(State state, AnchorInfo anchorInfo) {
        ...
        if (this.mPendingSavedState != null && this.mPendingSavedState.hasValidAnchor()) {
            ...
            if (findViewByPosition == null) {
                if (getChildCount() > 0) {
                    if ((this.mPendingScrollPosition < getPosition(getChildAt(0)) ? true : DEBUG) == this.mShouldReverseLayout) {
                        z = true;
                    }
    ...
    public void onLayoutChildren(Recycler recycler, State state) {
        ...
        if (getChildCount() > 0) {
            ...
            if ((this.mShouldReverseLayout ^ this.mStackFromEnd) != 0) {
                ...
                extraLayoutSpace += endAfterPadding;
                fixLayoutStartGap = fixLayoutStartGap(extraLayoutSpace, recycler, state, DEBUG);
                ...
                i += endAfterPadding;
                fixLayoutStartGap = fixLayoutEndGap(i, recycler, state, DEBUG);
